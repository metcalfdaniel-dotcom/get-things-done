...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `gtd-${Date.now()}.json`);
 ...
 * Applied at write points so GTD-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 * When a workstream is active (via explicit ws arg or GTD_WORKSTREAM env var),
 * returns `.planning/workstreams/{ws}/`. Otherwise returns `.planning/`.
 *
 * @param {string} cwd - project root
 * @param {string} [ws] - explicit workstream name; if omitted, checks GTD_WORKSTREAM env var
 */
 ...
 * gtd-tools.cjs lives at <configDir>/get-things-done/bin/gtd-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  // __dirname is get-things-done/bin/lib/ → go up 3 levels to configDir
