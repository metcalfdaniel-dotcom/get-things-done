...
Use gtd-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use gtd-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify artifacts "$plan")
...
Use gtd-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify key-links "$plan")
