...
- **Claude Code:** Uses `Task(subagent_type="gtd-executor", ...)` — blocks until complete, returns result
...
These are the valid GTD subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- gtd-executor — Executes plan tasks, commits, creates SUMMARY.md
...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-executor 2>/dev/null)
```
...
Check if the current runtime is Copilot by testing for the `@gtd-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
- Maintains GTD's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning GTD
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       For gtd-tools commits: add --no-verify flag.
...
5. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a GTD or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not GTD. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
