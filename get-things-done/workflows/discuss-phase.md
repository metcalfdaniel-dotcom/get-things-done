...
- Per-project: `gtd-tools config-set workflow.text_mode true`
...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-advisor 2>/dev/null)
```
...
Use /gsd:progress ${GTD_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /gsd:plan-phase {X} ${GTD_WS} to replan
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" resolve-model gtd-advisor-researcher --raw)
   ```
...
**Also available:**
- `/gsd:plan-phase ${PHASE} --skip-research ${GTD_WS}` — plan without research
- `/gsd:ui-phase ${PHASE} ${GTD_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-set workflow._auto_chain_active true
```
...
```
Skill(skill="gsd:plan-phase", args="${PHASE} --auto ${GTD_WS}")
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /gsd:execute-phase ${PHASE} ${GTD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /gsd:plan-phase ${PHASE} ${GTD_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /gsd:plan-phase ${PHASE} --gaps ${GTD_WS}
  ```
