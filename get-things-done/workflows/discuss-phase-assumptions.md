...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-assumptions-analyzer 2>/dev/null)
```
...
```
Task(subagent_type="gtd-assumptions-analyzer", prompt="""
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
