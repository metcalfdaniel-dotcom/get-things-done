...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing gtd-tools.cjs subcommands and the gtd-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
- @$HOME/.claude/get-things-done/agents/gtd-user-profiler.md (profiler agent definition)
...
```bash
cp "$HOME/.claude/get-things-done/USER-PROFILE.md" "$HOME/.claude/get-things-done/USER-PROFILE.backup.md"
```
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/get-things-done/bin/gtd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/get-things-done/bin/gtd-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/get-things-done/bin/gtd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/get-things-done/bin/gtd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $HOME/.claude/get-things-done/bin/gtd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/get-things-done/bin/gtd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gtd-dev-preferences at $HOME/.claude/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/get-things-done/bin/gtd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/get-things-done/bin/gtd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any gtd-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/get-things-done/USER-PROFILE.backup.md"
```
...
```
Artifacts:
  ✓ /gtd-dev-preferences   $HOME/.claude/commands/gsd/dev-preferences.md
...
- [ ] Artifacts generated sequentially via gtd-tools.cjs subcommands
