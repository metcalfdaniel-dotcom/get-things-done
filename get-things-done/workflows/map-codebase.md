...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-codebase-mapper 2>/dev/null)
```
...
Use Task tool with `subagent_type="gtd-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
- If Task tool available: 4 parallel gtd-codebase-mapper agents spawned with run_in_background=true
