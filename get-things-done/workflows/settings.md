<purpose>
Interactive configuration of GTD workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.gsd/defaults.json) for future projects.
</purpose>
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-ensure-section
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state load)
...
These settings apply to future /gtd-plan-phase and /gtd-execute-phase runs.
