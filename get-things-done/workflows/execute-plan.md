...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via gtd-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gtd-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn gtd-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" phases list --type summaries --raw
...
```bash
node ~/.claude/get-things-done/bin/gtd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
```bash
NODE_REPAIR=$(node "./.claude/get-things-done/bin/gtd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```
...
Update STATE.md using gtd-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state record-metric \
...
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using gtd-tools:

```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gtd-execute-phase {phase}` + `/gtd-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gtd-plan-phase {Z+1}` + `/gtd-verify-work {Z}` + `/gtd-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gtd-complete-milestone` + `/gtd-verify-work` + `/gtd-add-phase` |
