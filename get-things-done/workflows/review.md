...
**Gemini:**
```bash
gemini -p "$(cat /tmp/gtd-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gtd-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/gtd-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/gtd-review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/gtd-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gtd-review-codex-{phase}.md
```
...
Commit:
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
