...
```bash
# Get state snapshot
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
Do not ask for confirmation — the whole point of `/gtd-next` is zero-friction advancement.
</step>
