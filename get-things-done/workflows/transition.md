...
**Delegate ROADMAP.md and STATE.md updates to gtd-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gtd-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `gtd-tools phase complete`:**
...
if [ -n "$GTD_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" workstream list --raw)
fi
```
...
Exit skill and invoke SlashCommand("/gsd:plan-phase [X+1] --auto ${GTD_WS}")
...
Exit skill and invoke SlashCommand("/gsd:discuss-phase [X+1] --auto ${GTD_WS}")
...
`/gsd:discuss-phase [X+1] ${GTD_WS}` — gather context and clarify approach
...
**Also available:**
- `/gsd:plan-phase [X+1] ${GTD_WS}` — skip discussion, plan directly
- `/gsd:research-phase [X+1] ${GTD_WS}` — investigate unknowns
...
**Also available:**
- `/gsd:discuss-phase [X+1] ${GTD_WS}` — revisit context
- `/gsd:research-phase [X+1] ${GTD_WS}` — investigate unknowns
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/gsd:workstreams complete {current_ws_name} ${GTD_WS}`

See overall milestone progress:

`/gsd:workstreams progress ${GTD_WS}`
...
Exit skill and invoke SlashCommand("/gsd:complete-milestone {version} ${GTD_WS}")
...
`/gsd:complete-milestone {version} ${GTD_WS}`
