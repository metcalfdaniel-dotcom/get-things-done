...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init manager)
...
Steps:
1. Read the plan-phase workflow: cat ~/.claude/get-things-done/workflows/plan-phase.md
2. Run: node \"$HOME/.claude/get-things-done/bin/gtd-tools.cjs\" init plan-phase {N}
...
5. Spawn a gtd-planner subagent via Task() to create the plans.
6. If plan-checker is enabled, spawn a gtd-plan-checker subagent to verify.
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the GTD execute-phase workflow for phase {N} of the project.
...
Steps:
1. Read the execute-phase workflow: cat ~/.claude/get-things-done/workflows/execute-phase.md
2. Run: node \"$HOME/.claude/get-things-done/bin/gtd-tools.cjs\" init execute-phase {N}
3. Follow the workflow steps: discover plans, analyze dependencies, group into waves.
4. For each wave, spawn gtd-executor subagents via Task() to execute plans in parallel.
5. After all waves complete, spawn a gtd-verifier subagent if verifier is enabled.
