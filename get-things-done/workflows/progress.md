...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init progress)
...
```bash
DISCUSS_MODE=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
<step name="load">
**Use structured extraction from gtd-tools:**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap analyze)`
- `STATE=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
<step name="report">
**Generate progress bar from gtd-tools, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
Review: `/gsd:audit-uat ${GTD_WS}` — full cross-phase audit
Resume testing: `/gsd:verify-work {phase} ${GTD_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/gsd:plan-phase {phase-number} ${GTD_WS}`
...
`/gsd:discuss-phase {phase} ${GTD_WS}` — gather context and clarify approach
...
**Also available:**
- `/gsd:plan-phase {phase} ${GTD_WS}` — skip discussion, plan directly
- `/gsd:list-phase-assumptions {phase} ${GTD_WS}` — see Claude's assumptions
...
`/gsd:plan-phase {phase} --gaps ${GTD_WS}`
...
**Also available:**
- `/gsd:execute-phase {phase} ${GTD_WS}` — execute phase plans
- `/gsd:verify-work {phase} ${GTD_WS}` — run more UAT testing
...
`/gsd:verify-work {phase} ${GTD_WS}` — resume testing from where you left off
...
**Also available:**
- `/gsd:audit-uat ${GTD_WS}` — full cross-phase UAT audit
- `/gsd:execute-phase {phase} ${GTD_WS}` — execute phase plans
...
```bash
NEXT_PHASE_SECTION=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
...
`/gsd:discuss-phase {Z+1} ${GTD_WS}` — gather context and clarify approach
...
**Also available:**
- `/gsd:plan-phase {Z+1} ${GTD_WS}` — skip discussion, plan directly
- `/gsd:verify-work {Z} ${GTD_WS}` — user acceptance test before continuing
...
**Also available:**
- `/gsd:verify-work ${GTD_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/gsd:plan-phase [next] ${GTD_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/gsd:resume-work ${GTD_WS}`
  </step>
