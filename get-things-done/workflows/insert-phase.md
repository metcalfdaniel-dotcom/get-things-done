...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init phase-op "${after_phase}")
...
**Delegate the phase insertion to gtd-tools:**

```bash
RESULT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" phase insert "${after_phase}" "${description}")
```
...
- [ ] `gtd-tools phase insert` executed successfully
