...
AGENT_SKILLS_UI=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" resolve-model gtd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" resolve-model gtd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
