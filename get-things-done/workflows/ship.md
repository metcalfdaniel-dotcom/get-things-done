...
```bash
CONFIG=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
- /gtd-complete-milestone (if last phase in milestone)
- /gtd-progress (to see what's next)
...
- /gtd-complete-milestone — if all phases in milestone are done
- /gtd-progress — see overall project state
- /gtd-execute-phase {next} — continue to next phase
</offer_next>
