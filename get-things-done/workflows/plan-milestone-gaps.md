...
HIGHEST=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/gtd-execute-phase {N}` — if plans already exist
