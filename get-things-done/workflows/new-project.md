...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-project-researcher 2>/dev/null)
AGENT_SKILLS_SYNTHESIZER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-synthesizer 2>/dev/null)
AGENT_SKILLS_ROADMAPPER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-roadmapper 2>/dev/null)
```
...
```bash
mkdir -p .planning
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: initialize project" --files .planning/PROJECT.md
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" generate-claude-md
```

This ensures new projects get the default GTD workflow-enforcement guidance and current project context in `CLAUDE.md`.

**Commit roadmap (after approval or auto mode):**

```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md CLAUDE.md
```
...
```bash
PHASE1_SECTION=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase 1 2>/dev/null)
