...
```bash
npm view get-things-done-cc version 2>/dev/null
```
...
To update manually: `npx get-things-done-cc --global`
```
...
**If LOCAL install:**
```bash
npx -y get-things-done-cc@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y get-things-done-cc@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y get-things-done-cc@latest --claude --global
```
...
The SessionStart hook (`gtd-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
[View full changelog](https://github.com/gtd-build/get-things-done/blob/main/CHANGELOG.md)
```
</step>
