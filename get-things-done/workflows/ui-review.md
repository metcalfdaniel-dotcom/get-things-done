...
AGENT_SKILLS_UI_REVIEWER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-ui-reviewer 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `commit_docs`.

```bash
UI_AUDITOR_MODEL=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" resolve-model gtd-ui-auditor --raw)
```
...
**If `SUMMARY_FILES` empty:** Exit — "Phase {N} not executed. Run /gtd-execute-phase {N} first."
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
