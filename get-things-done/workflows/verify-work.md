...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```bash
CHECKPOINT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- `/gtd-execute-phase {next}` — Execute next phase
...
<downstream_consumer>
Output consumed by /gtd-execute-phase
...
`/clear` then `/gtd-execute-phase {phase} --gaps-only`
...
- [ ] Ready for `/gtd-execute-phase --gaps-only` when complete
</success_criteria>
