...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/gsd:new-milestone --reset-phase-numbers ${GTD_WS}`.
...
**Commit requirements:**
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
`/gsd:discuss-phase [N] ${GTD_WS}` — gather context and clarify approach

<sub>`/clear` first → fresh context window</sub>

Also: `/gsd:plan-phase [N] ${GTD_WS}` — skip discussion, plan directly
```
...
- [ ] User knows next step: `/gsd:discuss-phase [N] ${GTD_WS}`
