...
AGENT_SKILLS_AUDITOR=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" resolve-model gtd-nyquist-auditor --raw)
...
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
