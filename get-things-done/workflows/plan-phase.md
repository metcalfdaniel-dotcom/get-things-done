...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-checker 2>/dev/null)
```
...
```bash
PHASE_INFO=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
```bash
UI_PHASE_CFG=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```
Skill(skill="gsd:execute-phase", args="${PHASE} --auto --no-transition ${GTD_WS}")
```
