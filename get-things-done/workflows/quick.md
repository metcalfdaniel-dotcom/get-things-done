<purpose>
Execute small, ad-hoc tasks with GTD guarantees (atomic commits, STATE.md tracking). Quick mode spawns gtd-planner (quick mode) + gtd-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► QUICK TASK (DISCUSS + RESEARCH + FULL)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-planner 2>/dev/null)
AGENT_SKILLS_EXECUTOR=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-executor 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-checker 2>/dev/null)
AGENT_SKILLS_VERIFIER=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" agent-skills gtd-verifier 2>/dev/null)
```
...
Spawn gtd-executor with plan reference:
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
GTD > QUICK TASK COMPLETE (FULL MODE)
