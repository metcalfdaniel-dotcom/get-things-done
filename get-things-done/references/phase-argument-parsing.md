...
```bash
PHASE_INFO=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" find-phase "${PHASE}" --raw)
```
