...
*"By far the most powerful addition to my Claude Code. Nothing over-engineered. Literally just gets things done."*
...
GTD fixes that. It's the context engineering layer that makes Claude Code reliable. Describe your idea, let the system extract everything it needs to know, and let Claude Code get to work.
...
```bash
# Claude Code
npx get-things-done-cc --claude --global   # Install to ~/.claude/
npx get-things-done-cc --claude --local    # Install to ./.claude/

# OpenCode (open source, free models)
npx get-things-done-cc --opencode --global # Install to ~/.config/opencode/

# Gemini CLI
npx get-things-done-cc --gemini --global   # Install to ~/.gemini/

# Codex (skills-first)
npx get-things-done-cc --codex --global    # Install to ~/.codex/
npx get-things-done-cc --codex --local     # Install to ./.codex/

# Copilot (GitHub Copilot CLI)
npx get-things-done-cc --copilot --global  # Install to ~/.github/
npx get-things-done-cc --copilot --local   # Install to ./.github/

# Cursor CLI
npx get-things-done-cc --cursor --global      # Install to ~/.cursor/
npx get-things-done-cc --cursor --local       # Install to ./.cursor/

# Windsurf (Codeium, VS Code-based)
npx get-things-done-cc --windsurf --global    # Install to ~/.windsurf/
npx get-things-done-cc --windsurf --local     # Install to ./.windsurf/

# Antigravity (Google, skills-first, Gemini-based)
npx get-things-done-cc --antigravity --global # Install to ~/.gemini/antigravity/
npx get-things-done-cc --antigravity --local  # Install to ./.agent/

# All runtimes
npx get-things-done-cc --all --global      # Install to all directories
```

Use `--global` (`-g`) or `--local` (`-l`) to skip the location prompt.
Use `--claude`, `--opencode`, `--gemini`, `--codex`, `--copilot`, `--cursor`, `--windsurf`, `--antigravity`, or `--all` to skip the runtime prompt.
Use `--sdk` to also install the GTD SDK CLI (`gtd-sdk`) for headless autonomous execution.
...
GTD is designed for frictionless automation. Run Claude Code with:
...
> [!TIP]
> This is how GTD is intended to be used — stopping to approve `date` and `git commit` 50 times defeats the purpose.
...
Quick mode gives you GTD guarantees (atomic commits, state tracking) with a faster path:
...
Control how GTD handles branches during execution.
...
**Strategies:**
- **`none`** — Commits to current branch (default GTD behavior)
- **`phase`** — Creates a branch per phase, merges at phase completion
- **`milestone`** — Creates one branch for entire milestone, merges at completion

At milestone completion, GTD offers squash merge (recommended) or merge with history.
...
GTD includes defense-in-depth security since v1.27:
...
- **PreToolUse prompt guard hook** — `gtd-prompt-guard` scans writes to `.planning/` for embedded injection vectors (advisory, not blocking)
...
> [!NOTE]
> Because GTD generates markdown files that become LLM system prompts, any user-controlled text flowing into planning artifacts is a potential indirect prompt injection vector. These protections are designed to catch such vectors at multiple layers.

### Protecting Sensitive Files

GTD's codebase mapping and analysis commands read files to understand your project. **Protect files containing secrets** by adding them to Claude Code's deny list:
...
> [!IMPORTANT]
> GTD includes built-in protections against committing secrets, but defense-in-depth is best practice. Deny read access to sensitive files as a first line of defense.
...
npx get-things-done-cc --windsurf --global --uninstall
...
npx get-things-done-cc --windsurf --local --uninstall
npx get-things-done-cc --antigravity --local --uninstall
```

This removes all GTD commands, agents, hooks, and settings while preserving your other configurations.

---

## Community Ports

OpenCode, Gemini CLI, and Codex are now natively supported via `npx get-things-done-cc`.

These community ports pioneered multi-runtime support:

| Project | Platform | Description |
|---------|----------|-------------|
| [gtd-opencode](https://github.com/rokicool/gtd-opencode) | OpenCode | Original OpenCode adaptation |
| gtd-gemini (archived) | Gemini CLI | Original Gemini adaptation by uberfuzzy |
...
**Claude Code is powerful. GTD makes it reliable.**
