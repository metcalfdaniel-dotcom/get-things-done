...
const GTD_CODEX_MARKER = '# GTD Agent Configuration \u2014 managed by get-things-done installer';
const GTD_CODEX_HOOKS_OWNERSHIP_PREFIX = '# GTD codex_hooks ownership: ';

// Copilot instructions marker constants
const GTD_COPILOT_INSTRUCTIONS_MARKER = '<!-- GTD Configuration \u2014 managed by get-things-done installer -->';
const GTD_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /GTD Configuration -->';

const CODEX_AGENT_SANDBOX = {
  'gtd-executor': 'workspace-write',
...
Then re-run: npx get-things-done-cc@latest
`);
    process.exit(1);
  }
}
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all gsd: references → gtd-)
  c = c.replace(/gsd:/g, 'gtd-');
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all gsd: references → gtd-)
  c = c.replace(/gsd:/g, 'gtd-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize gsd: -> gtd-.
  // This preserves rendered "next step" commands like "/gtd-execute-phase 17".
  return content.replace(/gsd:/gi, 'gtd-');
}
...
function convertSlashCommandsToCodexSkillMentions(content) {
  ...
  converted = converted.replace(/\/gtd-help\b/g, '$gtd-help');
  return converted;
}
...
function stripCodexGsdAgentSections(content) {
  return content.replace(/^\[agents\.gtd-[^\]]+\]\n(?:(?!\[)[^\n]*\n?)*/gm, '');
}
...
function stripLeakedGsdCodexSections(content) {
  const leakedSections = getTomlTableSections(content)
    .filter((section) =>
      section.path.startsWith('agents.gtd-') ||
      (
        section.path === 'agents' &&
...
function mergeCopilotInstructions(filePath, gsdContent) {
  const gsdBlock = GTD_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    gsdContent.trim() + '\n' +
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const codexGsdPath = `${path.resolve(targetDir, 'get-things-done').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/get-things-done prefix so path resolves to codex GTD install
    content = content.replace(/~\/\.claude\/get-things-done\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.claude\/get-things-done\//g, codexGsdPath);
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false } = {}) {
  ...
  // Replace /gsd:command with /gtd-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/gsd:/g, '/gtd-');
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/gsd/debug/start.md -> command/gtd-debug-start.md
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      jsContent = jsContent.replace(/gsd:/gi, 'gtd-');
...
/**
 * Clean up orphaned files from previous GTD versions
 */
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/gtd-notify.sh',  // Removed in v1.6.x
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    // Codex/Cursor/Windsurf: remove skills/gtd-*/SKILL.md skill directories
    ...
    // Cursor: remove skills/gtd-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Windsurf: remove skills/gtd-*/ directories (same layout as Cursor skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
  ...
  // 2. Remove get-things-done directory
  const gsdDir = path.join(targetDir, 'get-things-done');
  if (fs.existsSync(gsdDir)) {
    fs.rmSync(gsdDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed get-things-done/`);
  }
  ...
  if (fs.existsSync(hooksDir)) {
    const gsdHooks = ['gtd-statusline.js', 'gtd-check-update.js', 'gtd-check-update.sh', 'gtd-context-monitor.js', 'gtd-prompt-guard.js'];
    let hookCount = 0;
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    // Remove GTD hooks from PostToolUse and AfterTool (Gemini uses AfterTool)
    for (const eventName of ['PostToolUse', 'AfterTool']) {
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName].filter(entry => {
          if (entry.hooks && Array.isArray(entry.hooks)) {
            const hasGsdHook = entry.hooks.some(h =>
              h.command && h.command.includes('gtd-context-monitor')
            );
            return !hasGsdHook;
          }
          return true;
    ...
    // Remove GTD hooks from PreToolUse and BeforeTool (Gemini uses BeforeTool)
    for (const eventName of ['PreToolUse', 'BeforeTool']) {
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName].filter(entry => {
          if (entry.hooks && Array.isArray(entry.hooks)) {
            const hasGsdHook = entry.hooks.some(h =>
              h.command && h.command.includes('gtd-prompt-guard')
            );
            return !hasGsdHook;
          }
          return true;
          ...
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              ...
              for (const key of keys) {
                if (key.includes('get-things-done')) {
 ...
 * This prevents permission prompts when GTD accesses the get-things-done directory
...
function configureOpencodePermissions(isGlobal = true) {
  ...
  const gsdPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/get-things-done/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/get-things-done/*`;
...
function writeManifest(configDir, runtime = 'claude') {
  ...
  const isCursor = runtime === 'cursor';
  const isWindsurf = runtime === 'windsurf';
  const gsdDir = path.join(configDir, 'get-things-done');
  ...
  for (const [rel, hash] of Object.entries(gsdHashes)) {
    manifest.files['get-things-done/' + rel] = hash;
  }
  ...
  if (!isCodex && !isCopilot) {
    ...
    if (fs.existsSync(hooksDir)) {
      for (const file of fs.readdirSync(hooksDir)) {
        if (file.startsWith('gtd-') && file.endsWith('.js')) {
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'copilot')
      ...
      : runtime === 'codex'
        ? '$gtd-reapply-patches'
        : runtime === 'cursor'
          ? 'gtd-reapply-patches (mention the skill name)'
          : '/gsd:reapply-patches';
    console.log('');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>get-things-done/bin/...
  ...
  if (isOpencode) {
    ...
    // Copy commands/gsd/*.md as command/gtd-*.md (flatten structure)
  ...
  // Copy get-things-done skill with path replacement
  const skillSrc = path.join(src, 'get-things-done');
  const skillDest = path.join(targetDir, 'get-things-done');
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime, false, isGlobal);
  if (verifyInstalled(skillDest, 'get-things-done')) {
    console.log(`  ${green}✓${reset} Installed get-things-done`);
  } else {
    failures.push('get-things-done');
  }
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    // Remove old GTD agents (gtd-*.md) before copying new ones
  ...
  const changelogDest = path.join(targetDir, 'get-things-done', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'get-things-done', 'VERSION');
  ...
  // targetDir is e.g. ~/.claude/get-things-done/, parent is the config dir
  const updateCacheFile = path.join(path.dirname(targetDir), 'cache', 'gtd-update-check.json');
  ...
  if (isCodex) {
    ...
    try {
      ...
      const updateCheckScript = path.resolve(targetDir, 'get-things-done', 'hooks', 'gtd-update-check.js').replace(/\\/g, '/');
      const hookBlock =
        `${eol}# GTD Hooks${eol}` +
        `[[hooks]]${eol}` +
      ...
      if (hasEnabledCodexHooksFeature(configContent) && !configContent.includes('gtd-update-check')) {
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'get-things-done', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'gtd-context-monitor.js')
    : 'node ' + dirName + '/hooks/gtd-context-monitor.js';
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'gtd-prompt-guard.js')
    : 'node ' + dirName + '/hooks/gtd-prompt-guard.js';
...
/**
 * Install the GTD SDK globally via npm.
 * @returns {boolean} true if install succeeded
 */
function installSdk() {
  const sdkVersion = pkg.version;
  const sdkPkg = `@gtd-build/sdk@${sdkVersion}`;
  console.log(`\n  ${cyan}Installing GTD SDK...${reset}`);
  console.log(`  ${dim}npm install -g ${sdkPkg}${reset}\n`);
  try {
    require('child_process').execSync(`npm install -g ${sdkPkg}`, { stdio: 'inherit' });
    console.log(`\n  ${green}✓${reset} GTD SDK installed (${cyan}gtd-sdk${reset} command available)`);
    return true;
  } catch (e) {
...
/**
 * Prompt the user to optionally install the GTD SDK.
...
function promptSdk(callback) {
  ...
  console.log(`
  ${yellow}Also install the GTD SDK?${reset}

  The SDK provides a standalone CLI for autonomous execution:
    ${dim}gtd-sdk init @prd.md${reset}    Bootstrap a project from a PRD
    ${dim}gtd-sdk auto${reset}            Run full autonomous lifecycle
    ${dim}gtd-sdk run "prompt"${reset}    Execute a milestone from text

  ${cyan}1${reset}) No
  ${cyan}2${reset}) Yes ${dim}(runs: npm install -g @gtd-build/sdk)${reset}
`);
