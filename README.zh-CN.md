...
所以我做了 GTD。复杂性在系统内部，不在你的工作流里。幕后是上下文工程、XML 提示格式、子代理编排、状态管理；你看到的是几个真能工作的命令。
...
GTD 解决的就是这个问题。它是让 Claude Code 变得可靠的上下文工程层。你只要描述想法，系统会自动提取它需要知道的一切，然后让 Claude Code 去干活。
...
> [!NOTE]
> Codex 安装走的是 skill 机制（`skills/gtd-*/SKILL.md`），不是自定义 prompt。
...
```bash
# Claude Code
npx get-things-done-cc --claude --global   # 安装到 ~/.claude/
npx get-things-done-cc --claude --local    # 安装到 ./.claude/

# OpenCode（开源，可用免费模型）
npx get-things-done-cc --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
npx get-things-done-cc --gemini --global   # 安装到 ~/.gemini/

# Codex（以 skills 为主）
npx get-things-done-cc --codex --global    # 安装到 ~/.codex/
npx get-things-done-cc --codex --local     # 安装到 ./.codex/

# Copilot（GitHub Copilot CLI）
npx get-things-done-cc --copilot --global  # 安装到 ~/.github/
npx get-things-done-cc --copilot --local   # 安装到 ./.github/

# Cursor CLI
npx get-things-done-cc --cursor --global   # 安装到 ~/.cursor/
npx get-things-done-cc --cursor --local    # 安装到 ./.cursor/

# Antigravity（Google，以 skills 为主，基于 Gemini）
npx get-things-done-cc --antigravity --global # 安装到 ~/.gemini/antigravity/
npx get-things-done-cc --antigravity --local  # 安装到 ./.agent/

# 所有运行时
npx get-things-done-cc --all --global      # 安装到所有目录
```
...
GTD 的设计目标是无摩擦自动化。运行 Claude Code 时建议使用：
...
> [!TIP]
> 这才是 GTD 的预期用法。连 `date` 和 `git commit` 都要来回确认 50 次，整个体验就废了。
...
> **已经有现成代码库？** 先运行 `/gtd-map-codebase`。它会并行拉起多个代理分析你的技术栈、架构、约定和风险点。之后 `/gtd-new-project` 就会真正“理解”你的代码库，提问会聚焦在你打算新增的部分，规划时也会自动加载你的现有模式。
...
如果一切通过，就进入下一步；如果哪里坏了，你不需要手动 debug，只要重新运行 `/gtd-execute-phase`，执行它自动生成的修复计划即可。
...
如果你希望在讨论阶段更快收集信息，可以用 `/gtd-discuss-phase <n> --batch`，一次回答一小组问题，而不是逐个问答。

每个阶段都会得到你的输入（discuss）、充分研究（plan）、干净执行（execute）和人工验证（verify）。上下文始终保持新鲜，质量也能持续稳定。

当所有阶段完成后，`/gtd-complete-milestone` 会归档当前里程碑并打 release tag。

接着用 `/gtd-new-milestone` 开启下一个版本。它和 `new-project` 流程相同，只是面向你现有的代码库。你描述下一步想构建什么，系统研究领域、梳理需求，再产出新的路线图。每个里程碑都是一个干净周期：定义 → 构建 → 发布。
...
快速模式保留 GTD 的核心保障（原子提交、状态跟踪），但路径更短：
...
| 命令 | 作用 |
|------|------|
| `/gtd-new-project [--auto]` | 完整初始化：提问 → 研究 → 需求 → 路线图 |
| `/gtd-discuss-phase [N] [--auto] [--analyze]` | 在规划前收集实现决策（`--analyze` 增加权衡分析） |
| `/gtd-plan-phase [N] [--auto] [--reviews]` | 为某个阶段执行研究 + 规划 + 验证（`--reviews` 加载代码库审查结果） |
| `/gtd-execute-phase <N>` | 以并行 wave 执行全部计划，完成后验证 |
| `/gtd-verify-work [N]` | 人工用户验收测试 ¹ |
| `/gtd-ship [N] [--draft]` | 从已验证的阶段工作创建 PR，自动生成 PR 描述 |
| `/gtd-fast <text>` | 内联处理琐碎任务——完全跳过规划，立即执行 |
| `/gtd-next` | 自动推进到下一个逻辑工作流步骤 |
| `/gtd-audit-milestone` | 验证里程碑是否达到完成定义 |
| `/gtd-complete-milestone` | 归档里程碑并打 release tag |
| `/gtd-new-milestone [name]` | 开始下一个版本：提问 → 研究 → 需求 → 路线图 |
| `/gtd-milestone-summary` | 从已完成的里程碑产物生成项目概览，用于团队上手 |
| `/gtd-forensics` | 对失败或卡住的工作流进行事后调查 |

### 工作流（Workstreams）

| 命令 | 作用 |
|------|------|
| `/gtd-workstreams list` | 显示所有工作流及其状态 |
| `/gtd-workstreams create <name>` | 创建命名空间工作流，用于并行里程碑工作 |
| `/gtd-workstreams switch <name>` | 切换当前活跃工作流 |
| `/gtd-workstreams complete <name>` | 完成并合并工作流 |

### 多项目工作区

| 命令 | 作用 |
|------|------|
| `/gtd-new-workspace` | 创建隔离工作区，包含仓库副本（worktree 或 clone） |
| `/gtd-list-workspaces` | 显示所有 GTD 工作区及其状态 |
| `/gtd-remove-workspace` | 移除工作区并清理 worktree |

### UI 设计

| 命令 | 作用 |
|------|------|
| `/gtd-ui-phase [N]` | 为前端阶段生成 UI 设计合约（UI-SPEC.md） |
| `/gtd-ui-review [N]` | 对已实现前端代码进行 6 维视觉审计 |

### 导航

| 命令 | 作用 |
|------|------|
| `/gtd-progress` | 我现在在哪？下一步是什么？ |
| `/gtd-next` | 自动检测状态并执行下一步 |
| `/gtd-help` | 显示全部命令和使用指南 |
| `/gtd-update` | 更新 GTD，并预览变更日志 |
| `/gtd-join-discord` | 加入 GTD Discord 社区 |

### Brownfield

| 命令 | 作用 |
|------|------|
| `/gtd-map-codebase` | 在 `new-project` 前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|------|------|
| `/gtd-add-phase` | 在路线图末尾追加 phase |
| `/gtd-insert-phase [N]` | 在 phase 之间插入紧急工作 |
| `/gtd-remove-phase [N]` | 删除未来 phase，并重编号 |
| `/gtd-list-phase-assumptions [N]` | 在规划前查看 Claude 打算采用的方案 |
| `/gtd-plan-milestone-gaps` | 为 audit 发现的缺口创建 phase |

### 代码质量

| 命令 | 作用 |
|------|------|
| `/gtd-review` | 对当前阶段或分支进行跨 AI 同行评审 |
| `/gtd-pr-branch` | 创建过滤 `.planning/` 提交的干净 PR 分支 |
| `/gtd-audit-uat` | 审计验证债务——找出缺少 UAT 的阶段 |

### 积压

| 命令 | 作用 |
|------|------|
| `/gtd-plant-seed <idea>` | 将想法存入积压停车场，留待未来里程碑 |

### 会话

| 命令 | 作用 |
|------|------|
| `/gtd-pause-work` | 在中途暂停时创建交接上下文（写入 HANDOFF.json） |
| `/gtd-resume-work` | 从上一次会话恢复 |
| `/gtd-session-report` | 生成会话摘要，包含已完成工作和结果 |

### 工具

| 命令 | 作用 |
|------|------|
| `/gtd-settings` | 配置模型 profile 和工作流代理 |
| `/gtd-set-profile <profile>` | 切换模型 profile（quality / balanced / budget / inherit） |
| `/gtd-add-todo [desc]` | 记录一个待办想法 |
| `/gtd-check-todos` | 查看待办列表 |
| `/gtd-debug [desc]` | 使用持久状态进行系统化调试 |
| `/gtd-do <text>` | 将自由文本自动路由到正确的 GTD 命令 |
| `/gtd-note <text>` | 零摩擦想法捕捉——追加、列出或提升为待办 |
| `/gtd-quick [--full] [--discuss] [--research]` | 以 GTD 保障执行临时任务（`--full` 增加计划检查和验证，`--discuss` 先补上下文，`--research` 在规划前先调研） |
| `/gtd-health [--repair]` | 校验 `.planning/` 目录完整性，带 `--repair` 时自动修复 |
| `/gtd-stats` | 显示项目统计——阶段、计划、需求、git 指标 |
| `/gtd-profile-user [--questionnaire] [--refresh]` | 从会话分析生成开发者行为档案，用于个性化响应 |
...
GTD 将项目设置保存在 `.planning/config.json`。你可以在 `/gtd-new-project` 时配置，也可以稍后通过 `/gtd-settings` 修改。完整的配置 schema、工作流开关、git branching 选项以及各代理的模型分配，请查看[用户指南](docs/USER-GUIDE.md#configuration-reference)。
...
可以用 `/gtd-settings` 开关这些项，也可以在单次命令里覆盖：
...
**策略说明：**
- **`none`**：直接提交到当前分支（GTD 默认行为）
- **`phase`**：每个 phase 创建一个分支，在 phase 完成时合并
- **`milestone`**：整个里程碑只用一个分支，在里程碑完成时合并

在里程碑完成时，GTD 会提供 squash merge（推荐）或保留历史的 merge 选项。
...
GTD 的代码库映射和分析命令会读取文件来理解你的项目。**包含机密信息的文件应当加入 Claude Code 的 deny list**：
...
> [!IMPORTANT]
> GTD 内建了防止提交 secrets 的保护，但纵深防御依然是最佳实践。第一道防线应该是直接禁止读取敏感文件。
...
- 对 Codex，检查 skills 是否存在于 `~/.codex/skills/gtd-*/SKILL.md`（全局）或 `./.codex/skills/gtd-*/SKILL.md`（本地）

**命令行为不符合预期？**
- 运行 `/gtd-help` 确认安装成功
- 重新执行 `npx get-things-done-cc` 进行重装
...
这会移除所有 GTD 命令、代理、hooks 和设置，但会保留你其他配置。

---

## 社区移植版本

OpenCode、Gemini CLI 和 Codex 现在都已经通过 `npx get-things-done-cc` 获得原生支持。

这些社区移植版本曾率先探索多运行时支持：

| Project | Platform | Description |
|---------|----------|-------------|
| [gtd-opencode](https://github.com/rokicool/gtd-opencode) | OpenCode | 最初的 OpenCode 适配版本 |
| gtd-gemini (archived) | Gemini CLI | uberfuzzy 制作的最初 Gemini 适配版本 |
