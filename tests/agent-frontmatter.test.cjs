...
const WORKFLOWS_DIR = path.join(__dirname, '..', 'get-things-done', 'workflows');
...
describe('CLAUDEMD: CLAUDE.md compliance enforcement', () => {
  ...
  test('gtd-executor has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-executor.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'gtd-executor must enforce CLAUDE.md directives during execution'
    );
    assert.ok(
      content.includes('CLAUDE.md rule — it takes precedence over plan instructions'),
      'gtd-executor must specify CLAUDE.md precedence over plan instructions'
    );
  });

  test('all three agents read CLAUDE.md in project_context', () => {
    const agents = ['gtd-plan-checker', 'gtd-phase-researcher', 'gtd-executor'];
...
describe('DISCUSS: discussion log generation', () => {
  ...
  test('discussion-log template exists', () => {
    const templatePath = path.join(__dirname, '..', 'get-things-done', 'templates', 'discussion-log.md');
    assert.ok(
...
describe('PERM: worktree agents have permissionMode: acceptEdits', () => {
  ...
  const WORKTREE_AGENTS = ['gtd-executor', 'gtd-debugger'];
  ...
  test('worktree-spawned agents are covered', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        const content = fs.readFileSync(path.join(dir, file), 'utf-8');
        // Find patterns like: subagent_type="gtd-executor" ... isolation="worktree"
