...
const MODEL_PROFILES = require('../get-things-done/bin/lib/model-profiles.cjs').MODEL_PROFILES;
 ...
 * produces. gtd-tools.cjs lives at <configDir>/get-things-done/bin/gtd-tools.cjs,
...
describe('init commands: agents_installed field (#1371)', () => {
  ...
  test('init execute-phase includes agents_installed=true when agents exist', () => {
    ...
    // Create agents dir as sibling of get-things-done/ (the installed layout)
    // gtd-tools.cjs resolves agents from GTD_INSTALL_DIR or __dirname/../../agents
    const gsdInstallDir = path.resolve(__dirname, '..', 'get-things-done', 'bin');
    const configDir = path.resolve(gsdInstallDir, '..', '..');
    const agentsDir = path.join(configDir, 'agents');

    // Agents already exist in the repo root /agents/ dir which is sibling to get-things-done/
...
describe('validate health: agent installation check W010 (#1371)', () => {
  ...
  test('health check reports healthy when agents are installed (repo layout)', () => {
    // In the repo, agents/ exists as a sibling of get-things-done/, so the
