...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('gtd-execute-phase');
...
You are a GTD plan executor.
    ...
    assert.ok(result.includes('"gtd-executor"'), 'has quoted name');
    ...
    assert.ok(result.includes('role: gtd-executor'), 'role matches agent name');
...
Run /gsd:execute-phase to proceed.`;
    ...
    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$gtd-execute-phase'), 'converts slash commands');
...
<role>You are an executor.</role>`;
  ...
  test('sets workspace-write for executor', () => {
    const result = generateCodexAgentToml('gtd-executor', sampleAgent);
...
<role>You check plans.</role>`;
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('gtd-executor', sampleAgent);
    assert.ok(result.includes('name = "gtd-executor"'), 'has name');
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'gtd-executor', 'gtd-planner', 'gtd-phase-researcher',
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'gtd-executor', description: 'Executes plans' },
  ...
  test('does not include feature flags or agents table header', () => {
    ...
    // Should not have bare [agents] table header (only [agents.gtd-*] sections)
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.gtd-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.gtd-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/gtd-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/gtd-executor.toml"'), 'absolute executor path');
...
describe('stripGsdFromCodexConfig', () => {
  ...
  test('removes [agents.gtd-*] sections', () => {
    const content = `[agents.gtd-executor]\ndescription = "test"\nconfig_file = "agents/gtd-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.ok(!result.includes('[agents.gtd-executor]'), 'removes GTD agent section');
...
describe('mergeCodexConfig', () => {
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes('[agents.gtd-executor]'), 'has agent');
  ...
  test('case 2: replaces existing GTD block', () => {
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'gtd-executor', description: 'Updated description' },
    ...
    assert.ok(content.includes('[agents.gtd-planner]'), 'has new agent');
  ...
  test('case 3 strips existing [agents.gtd-*] sections before appending fresh block', () => {
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    const gsdAgentCount = (content.match(/^\[agents\.gtd-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(GTD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-GTD agent section');
    assert.strictEqual(gsdAgentCount, 1, 'keeps exactly one GTD agent section');
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.gtd-executor]'), 'has agent from fresh block');
  });
  ...
  test('case 2 strips leaked GTD-managed sections above marker in CRLF files', () => {
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.gtd-executor\]\s*$/gm), 0, 'removes leaked GTD agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.gtd-executor\]\s*$/gm), 1, 'keeps one managed agent section');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 preserves user-authored [agents] tables while stripping leaked GTD sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.gtd-executor\]\s*$/gm), 0, 'removes leaked GTD agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.gtd-executor\]\s*$/gm), 1, 'keeps one managed agent section in the GTD block');
    assertUsesOnlyEol(content, '\r\n');
  });
...
describe('installCodexConfig (integration)', () => {
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes('[agents.gtd-executor]'), 'has executor agent');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'gtd-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'gtd-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'gtd-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "gtd-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes GTD plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by execute-phase orchestrator or execute-plan command."'), 'executor has description');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(content.includes('# GTD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes GTD SessionStart hook block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'writes one GTD update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'adds one GTD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.gtd-codebase-mapper]'), 'keeps codex_hooks before the next real table');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'does not duplicate GTD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'keeps one GTD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'remains idempotent for the GTD hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'adds one GTD update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 0, 'does not add the GTD hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.gtd-executor]'), 'still installs the managed agent block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'adds one GTD update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'adds the GTD update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# GTD Agent Configuration — managed by get-things-done installer\n`), 'writes the managed agent block using the first newline style');
    assert.ok(content.includes('# GTD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes the GTD hook block using the first newline style');
    ...
    assert.strictEqual(countMatches(content, /gtd-update-check\.js/g), 1, 'does not duplicate the GTD hook block');
    assertNoDraftRootKeys(content);
  });
});
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /gtd-update-check\.js/g), 0, 'removes only the GTD update hook');
  ...
  test('install then uninstall removes dotted features.codex_hooks without creating a [features] table', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-GTD hooks');
    assert.strictEqual(countMatches(cleaned, /gtd-update-check\.js/g), 0, 'removes the GTD update hook');
  });
