...
| `/gsd:quick [--full] [--discuss]` | 用 GTD 保证执行临时任务（`--full` 添加计划检查和验证，`--discuss` 先收集上下文） |
...
GTD 在 `.planning/config.json` 中存储项目设置。在 `/gsd:new-project` 期间配置或稍后用 `/gsd:settings` 更新。完整配置模式、工作流开关、git 分支选项和每个代理的模型分解，请参阅[用户指南](USER-GUIDE.md#配置参考)。
