...
| `/gsd:update` | 更新 GTD 并预览变更日志 | 检查新版本 |
...
| `/gsd:quick` | 带 GTD 保证的临时任务 | Bug 修复、小功能、配置更改 |
...
GTD 在 `.planning/config.json` 中存储项目设置。在 `/gsd:new-project` 期间配置或稍后用 `/gsd:settings` 更新。
...
在主要命令之间清除上下文窗口：Claude Code 中的 `/clear`。GTD 设计围绕全新上下文 —— 每个子代理获得干净的 200K 窗口。如果主会话质量下降，清除并使用 `/gsd:resume-work` 或 `/gsd:progress` 恢复状态。
...
从 v1.17 开始，安装程序将本地修改的文件备份到 `gtd-local-patches/`。运行 `/gsd:reapply-patches` 将你的更改合并回来。
