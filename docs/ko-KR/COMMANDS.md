...
```bash
/gtd-new-project                    # 대화형 모드
/gtd-new-project --auto @prd.md     # PRD에서 자동 추출
```
...
/gtd-new-workspace --name feature-b --repos . --strategy worktree  # 동일 저장소 격리
/gtd-new-workspace --name spike --repos api,web --strategy clone   # 전체 클론
```
...
활성 GTD 워크스페이스와 상태를 목록으로 표시합니다.

**스캔 위치:** `~/gtd-workspaces/`에서 `WORKSPACE.md` 매니페스트를 탐색합니다.
**표시 항목:** 이름, 저장소 수, 전략, GTD 프로젝트 상태
...
```bash
/gtd-discuss-phase 1                # 페이즈 1 대화형 토론
/gtd-discuss-phase 3 --auto         # 페이즈 3 기본값 자동 선택
/gtd-discuss-phase --batch          # 현재 페이즈 일괄 모드
/gtd-discuss-phase 2 --analyze      # 트레이드오프 분석 포함 토론
```
...
```bash
/gtd-plan-phase 1                   # 페이즈 1 조사 + 계획 + 검증
/gtd-plan-phase 3 --skip-research   # 조사 없이 계획 (익숙한 도메인)
/gtd-plan-phase --auto              # 비대화형 계획 수립
```
...
```bash
/gtd-execute-phase 1                # 페이즈 1 실행
/gtd-execute-phase 1 --wave 2       # Wave 2만 실행
```
...
**동작 방식.**
- 프로젝트 없음 → `/gtd-new-project` 제안
- 페이즈 토론 필요 → `/gtd-discuss-phase` 실행
- 페이즈 계획 필요 → `/gtd-plan-phase` 실행
- 페이즈 실행 필요 → `/gtd-execute-phase` 실행
- 페이즈 검증 필요 → `/gtd-verify-work` 실행
...
```bash
/gtd-session-report                 # 세션 종료 후 요약 생성
```
...
**사전 조건:** 페이즈 검증 완료 (`/gtd-verify-work` 통과), `gh` CLI 설치 및 인증
**생성 파일:** 계획 아티팩트 기반의 풍부한 본문이 포함된 GitHub PR, STATE.md 업데이트

```bash
/gtd-ship 4                         # 페이즈 4 출시
/gtd-ship 4 --draft                 # 초안 PR로 출시
```
...
**사전 조건:** 프론트엔드 코드가 있는 프로젝트 (독립 실행 가능, GTD 프로젝트 불필요)
**생성 파일:** `{phase}-UI-REVIEW.md`, `.planning/ui-reviews/`에 스크린샷

```bash
/gtd-ui-review                      # 현재 페이즈 감사
/gtd-ui-review 3                    # 페이즈 3 감사
```
...
```bash
/gtd-milestone-summary                # 현재 마일스톤 요약
/gtd-milestone-summary v1.0           # 특정 마일스톤 요약
```
...
```bash
/gtd-new-milestone                  # 대화형 모드
/gtd-new-milestone "v2.0 Mobile"    # 이름이 지정된 마일스톤
/gtd-new-milestone --reset-phase-numbers "v2.0 Mobile"  # 마일스톤 번호를 1부터 재시작
```
...
```bash
/gtd-insert-phase 3                 # 페이즈 3과 4 사이에 삽입 → 3.1 생성
```
...
```bash
/gtd-remove-phase 7                 # 페이즈 7 제거, 8→7, 9→8 등으로 재번호
```
...
```bash
/gtd-list-phase-assumptions 2       # 페이즈 2 가정 사항 확인
```
...
```bash
/gtd-plan-milestone-gaps             # 각 감사 갭에 대한 페이즈 생성
```

### `/gtd-research-phase`

심층 에코시스템 조사만 수행합니다 (독립 실행 — 일반적으로 `/gtd-plan-phase`를 사용하세요).
...
```bash
/gtd-research-phase 4               # 페이즈 4 도메인 조사
```
...
```bash
/gtd-validate-phase 2               # 페이즈 2 테스트 커버리지 감사
```
...
```bash
/gtd-progress                       # "지금 어디 있나? 다음은 무엇인가?"
```
...
```bash
/gtd-resume-work                    # 컨텍스트 초기화 또는 새 세션 후 실행
```
...
```bash
/gtd-pause-work                     # continue-here.md 생성
```
...
```bash
/gtd-quick                          # 기본 빠른 작업
/gtd-quick --discuss --research     # 토론 + 조사 + 계획
/gtd-quick --full                   # 계획 검사 및 검증 포함
/gtd-quick --discuss --research --full  # 모든 선택적 단계 포함
```
...
```bash
/gtd-autonomous                     # 남은 모든 페이즈 실행
/gtd-autonomous --from 3            # 페이즈 3부터 시작
```

### `/gtd-do`

자유 형식 텍스트를 적절한 GTD 명령어로 라우팅합니다.

```bash
/gtd-do                             # 원하는 작업을 설명합니다
```
...
- `/gtd-dev-preferences` 명령어 — 모든 세션에서 선호도를 로드합니다
- `CLAUDE.md` 프로필 섹션 — Claude Code에 의해 자동으로 인식됩니다

```bash
/gtd-profile-user                   # 세션 분석 및 프로필 구축
/gtd-profile-user --questionnaire   # 대화형 설문지 대체 방법
/gtd-profile-user --refresh         # 새로운 분석으로 재생성
```
...
실패하거나 멈춘 GTD 워크플로우에 대한 사후 조사를 수행합니다.
...
```bash
/gtd-forensics                              # 대화형 — 문제 입력 프롬프트
/gtd-forensics "Phase 3 execution stalled"  # 문제 설명과 함께 실행
```
...
```bash
/gtd-workstreams                    # 모든 워크스트림 목록
/gtd-workstreams create backend-api # 새 워크스트림 생성
/gtd-workstreams switch backend-api # 활성 워크스트림 설정
/gtd-workstreams status backend-api # 상세 상태 확인
/gtd-workstreams progress           # 교차 워크스트림 진행 상황 개요
/gtd-workstreams complete backend-api  # 완료된 워크스트림 아카이브
/gtd-workstreams resume backend-api    # 워크스트림 작업 재개
```
...
```bash
/gtd-set-profile budget             # 예산 프로필로 전환
/gtd-set-profile quality            # 품질 프로필로 전환
```
...
```bash
/gtd-map-codebase                   # 전체 코드베이스 분석
/gtd-map-codebase auth              # 인증 영역에 집중
```
...
변경 로그 미리보기와 함께 GTD를 업데이트합니다.

```bash
/gtd-update                         # 업데이트 확인 및 설치
```

### `/gtd-reapply-patches`

GTD 업데이트 후 로컬 수정사항을 복원합니다.

```bash
/gtd-reapply-patches                # 로컬 변경사항 병합
```
...
**`/gtd-quick`의 대체가 아닙니다.** 조사, 다단계 계획 또는 검증이 필요한 작업에는 `/gtd-quick`을 사용하세요.
...
**생성 파일:** `{phase}-REVIEWS.md` — `/gtd-plan-phase --reviews`에서 사용 가능
...
**목적:** 리뷰어에게 GTD 계획 아티팩트가 아닌 코드 변경사항만 표시합니다.

```bash
/gtd-pr-branch                     # main을 기준으로 필터링
/gtd-pr-branch develop             # develop을 기준으로 필터링
```
...
**999.x 번호 체계**는 백로그 항목을 활성 페이즈 순서 밖에 유지합니다. 페이즈 디렉터리가 즉시 생성되므로 해당 항목에 대해 `/gtd-discuss-phase`와 `/gtd-plan-phase`를 사용할 수 있습니다.
...
**사용처:** `/gtd-new-milestone` (시드를 스캔하여 일치 항목 제시)
...
스레드는 여러 세션에 걸쳐 이어지지만 특정 페이즈에 속하지 않는 작업을 위한 경량 교차 세션 지식 저장소입니다. `/gtd-pause-work`보다 가볍습니다.

```bash
/gtd-thread                         # 모든 스레드 목록
/gtd-thread fix-deploy-key-auth     # 스레드 재개
/gtd-thread "Investigate TCP timeout in pasta service"  # 새 스레드 생성
```
