...
GTD는 사용자와 AI 코딩 에이전트(Claude Code, Gemini CLI, OpenCode, Codex, Copilot, Antigravity) 사이에 위치하는 **메타 프롬프팅 프레임워크**입니다. 다음을 제공합니다.
...
워크플로우 파일(`get-things-done/workflows/*.md`)은 무거운 작업을 직접 수행하지 않습니다. 다음 작업만 담당합니다.
- `gtd-tools.cjs init <workflow>`로 컨텍스트를 로드합니다
...
- `gtd-tools.cjs init`을 통한 컨텍스트 로드
...
모든 계획 아티팩트를 위한 Markdown 템플릿입니다. `gtd-tools.cjs template fill`과 `scaffold` 명령어가 사전 구조화된 파일을 생성하는 데 사용합니다.
...
| 훅 | 이벤트 | 목적 |
|------|-------|---------|
| `gtd-statusline.js` | `statusLine` | 모델, 작업, 디렉터리, 컨텍스트 사용 바 표시 |
| `gtd-context-monitor.js` | `PostToolUse` / `AfterTool` | 잔여 35%/25% 시점에 에이전트 대면 컨텍스트 경고 주입 |
| `gtd-check-update.js` | `SessionStart` | 새 GTD 버전을 백그라운드에서 확인 |
| `gtd-prompt-guard.js` | `PreToolUse` | `.planning/` 쓰기 작업에서 프롬프트 인젝션 패턴 스캔 (권고용) |
| `gtd-workflow-guard.js` | `PreToolUse` | GTD 워크플로우 컨텍스트 외부의 파일 편집 감지 (권고용, `hooks.workflow_guard`로 활성화) |

### CLI Tools (`get-things-done/bin/`)

17개의 도메인 모듈을 포함하는 Node.js CLI 유틸리티(`gtd-tools.cjs`)입니다.
...
| 카테고리 | 에이전트 | 병렬성 |
|----------|--------|-------------|
| **Researchers** | gtd-project-researcher, gtd-phase-researcher, gtd-ui-researcher, gtd-advisor-researcher | 4개 병렬 (stack, features, architecture, pitfalls); advisor는 discuss-phase 중 생성됨 |
| **Synthesizers** | gtd-research-synthesizer | 순차적 (조사자 완료 후) |
| **Planners** | gtd-planner, gtd-roadmapper | 순차적 |
| **Checkers** | gtd-plan-checker, gtd-integration-checker, gtd-ui-checker, gtd-nyquist-auditor | 순차적 (검증 루프, 최대 3회 반복) |
| **Executors** | gtd-executor | 웨이브 내 병렬, 웨이브 간 순차적 |
| **Verifiers** | gtd-verifier | 순차적 (모든 executor 완료 후) |
| **Mappers** | gtd-codebase-mapper | 4개 병렬 (tech, arch, quality, concerns) |
| **Debuggers** | gtd-debugger | 순차적 (대화형) |
| **Auditors** | gtd-ui-auditor | 순차적 |
...
├── hooks/
│   ├── gtd-statusline.js           # 상태표시줄 훅
│   ├── gtd-context-monitor.js      # 컨텍스트 경고 훅
│   └── gtd-check-update.js         # 업데이트 확인 훅
├── settings.json                   # 훅 등록
└── VERSION                         # 설치된 버전 번호
```
...
GTD는 통합된 명령어/워크플로우 아키텍처를 통해 6개의 AI 코딩 런타임을 지원합니다.
...
5. **모델 참조** — `inherit` 프로필을 통해 GTD가 런타임의 모델 선택에 위임합니다
