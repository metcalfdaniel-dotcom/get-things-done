...
**목표:** `/gtd-new-project`가 `.planning/config.json`을 생성할 때, 파일에 사용자가 선택한 6개 키만이 아닌 모든 유효한 기본값이 포함되도록 하여 개발자가 소스 코드를 읽지 않고도 모든 설정을 확인할 수 있게 합니다.
 ...
 *
 * 사용자가 선택한 설정을 JSON 문자열로 받습니다(/gtd-new-project 중 명시적으로
...
**문제:**
`/gtd-new-project`는 온보딩 중 사용자가 명시적으로 선택한 6개 키만으로
...
2. **암묵적 확장**: `/gtd-settings` 또는 `config-set`이 처음으로 config에
