# Referência de Recursos do GTD

Visão em Português dos recursos centrais do GTD.  
...
- **UAT guiado** via `/gtd-verify-work`
- **Suporte brownfield** com `/gtd-map-codebase`
...
- **`/gtd-ui-phase`**: contrato visual antes da execução
- **`/gtd-ui-review`**: auditoria visual em 6 pilares
...
- **Diagnóstico forense** com `/gtd-forensics`
- **Relatório de sessão** com `/gtd-session-report`
...
| Cenário | Comandos |
|--------|----------|
| Projeto novo | `/gtd-new-project` -> `/gtd-discuss-phase` -> `/gtd-plan-phase` -> `/gtd-execute-phase` |
| Correção rápida | `/gtd-quick` |
| Código existente | `/gtd-map-codebase` -> `/gtd-new-project` |
| Fechamento de release | `/gtd-audit-milestone` -> `/gtd-complete-milestone` |
