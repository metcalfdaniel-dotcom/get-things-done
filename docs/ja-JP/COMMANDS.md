...
```bash
/gtd-new-project                    # 対話モード
/gtd-new-project --auto @prd.md     # PRDから自動抽出
```
...
/gtd-new-workspace --name feature-b --repos . --strategy worktree  # 同一リポジトリの分離
/gtd-new-workspace --name spike --repos api,web --strategy clone   # フルクローン
```
...
アクティブなGTDワークスペースとそのステータスを一覧表示します。

**スキャン対象:** `~/gtd-workspaces/` 内の `WORKSPACE.md` マニフェスト
**表示内容:** 名前、リポジトリ数、戦略、GTDプロジェクトのステータス
...
```bash
/gtd-discuss-phase 1                # フェーズ1の対話的ディスカッション
/gtd-discuss-phase 3 --auto         # フェーズ3でデフォルトを自動選択
/gtd-discuss-phase --batch          # 現在のフェーズのバッチモード
/gtd-discuss-phase 2 --analyze      # トレードオフ分析付きディスカッション
```
...
```bash
/gtd-plan-phase 1                   # フェーズ1の調査＋計画＋検証
/gtd-plan-phase 3 --skip-research   # 調査なしで計画（馴染みのあるドメイン）
/gtd-plan-phase --auto              # 非対話型の計画
```
...
```bash
/gtd-execute-phase 1                # フェーズ1を実行
/gtd-execute-phase 1 --wave 2       # ウェーブ2のみを実行
```
...
**動作:**
- プロジェクトなし → `/gtd-new-project` を提案
- フェーズにディスカッションが必要 → `/gtd-discuss-phase` を実行
- フェーズに計画が必要 → `/gtd-plan-phase` を実行
- フェーズに実行が必要 → `/gtd-execute-phase` を実行
- フェーズに検証が必要 → `/gtd-verify-work` を実行
- 全フェーズ完了 → `/gtd-complete-milestone` を提案
...
```bash
/gtd-session-report                 # セッション後のサマリーを生成
```
...
**前提条件:** フェーズが検証済み（`/gtd-verify-work` が合格）、`gh` CLIがインストールされ認証済みであること
**生成物:** 計画アーティファクトからリッチな本文を持つGitHub PR、STATE.mdの更新

```bash
/gtd-ship 4                         # フェーズ4をシップ
/gtd-ship 4 --draft                 # ドラフトPRとしてシップ
```
...
**前提条件:** プロジェクトにフロントエンドコードがあること（単体で動作、GTDプロジェクト不要）
**生成物:** `{phase}-UI-REVIEW.md`、`.planning/ui-reviews/` 内のスクリーンショット

```bash
/gtd-ui-review                      # 現在のフェーズを監査
/gtd-ui-review 3                    # フェーズ3を監査
```
...
```bash
/gtd-milestone-summary                # 現在のマイルストーンをサマリー
/gtd-milestone-summary v1.0           # 特定のマイルストーンをサマリー
```
...
```bash
/gtd-new-milestone                  # 対話モード
/gtd-new-milestone "v2.0 Mobile"    # 名前付きマイルストーン
/gtd-new-milestone --reset-phase-numbers "v2.0 Mobile"  # マイルストーン番号を1からリスタート
```
...
```bash
/gtd-insert-phase 3                 # フェーズ3と4の間に挿入 → 3.1を作成
```
...
```bash
/gtd-remove-phase 7                 # フェーズ7を削除、8→7、9→8等に番号振り直し
```
...
```bash
/gtd-list-phase-assumptions 2       # フェーズ2の前提を確認
```
...
```bash
/gtd-plan-milestone-gaps             # 各監査ギャップに対してフェーズを作成
```

### `/gtd-research-phase`

詳細なエコシステム調査のみを実行します（単体機能 — 通常は `/gtd-plan-phase` を使用してください）。
...
```bash
/gtd-research-phase 4               # フェーズ4のドメインを調査
```
...
```bash
/gtd-validate-phase 2               # フェーズ2のテストカバレッジを監査
```
...
```bash
/gtd-resume-work                    # コンテキストリセットまたは新しいセッション後に使用
```
...
```bash
/gtd-pause-work                     # continue-here.mdを作成
```
...
```bash
/gtd-manager                        # コマンドセンターダッシュボードを開く
```
...
```bash
/gtd-quick                          # 基本的なクイックタスク
/gtd-quick --discuss --research     # ディスカッション＋調査＋計画
/gtd-quick --full                   # プランチェックと検証付き
/gtd-quick --discuss --research --full  # すべてのオプションステージ
```
...
```bash
/gtd-autonomous                     # 残りの全フェーズを実行
/gtd-autonomous --from 3            # フェーズ3から開始
```

### `/gtd-do`

フリーテキストを適切なGTDコマンドにルーティングします。

```bash
/gtd-do                             # その後、やりたいことを説明
```
...
- `/gtd-dev-preferences` コマンド — 任意のセッションでプリファレンスをロード
- `CLAUDE.md` プロファイルセクション — Claude Codeが自動検出

```bash
/gtd-profile-user                   # セッションを分析してプロファイルを構築
/gtd-profile-user --questionnaire   # 対話型アンケートのフォールバック
/gtd-profile-user --refresh         # 新鮮な分析からの再生成
```
...
```bash
/gtd-forensics                              # 対話型 — 問題の入力を促す
/gtd-forensics "Phase 3 execution stalled"  # 問題の説明付き
```
...
**前提条件:** アクティブなGTDプロジェクト
**生成物:** `.planning/` 配下のワークストリームディレクトリ、ワークストリームごとの状態追跡

```bash
/gtd-workstreams                    # すべてのワークストリームを一覧表示
/gtd-workstreams create backend-api # 新しいワークストリームを作成
/gtd-workstreams switch backend-api # アクティブなワークストリームを設定
/gtd-workstreams status backend-api # 詳細ステータス
/gtd-workstreams progress           # ワークストリーム横断の進捗概要
/gtd-workstreams complete backend-api  # 完了したワークストリームをアーカイブ
/gtd-workstreams resume backend-api    # ワークストリームでの作業を再開
```
...
```bash
/gtd-set-profile budget             # budgetプロファイルに切り替え
/gtd-set-profile quality            # qualityプロファイルに切り替え
```
...
```bash
/gtd-map-codebase                   # コードベース全体を分析
/gtd-map-codebase auth              # auth領域にフォーカス
```
...
変更履歴のプレビュー付きでGTDをアップデートします。

```bash
/gtd-update                         # アップデートを確認してインストール
```

### `/gtd-reapply-patches`

GTDアップデート後にローカルの変更を復元します。

```bash
/gtd-reapply-patches                # ローカルの変更をマージバック
```
...
**`/gtd-quick` の代替ではありません** — 調査、複数ステップの計画、または検証が必要な場合は `/gtd-quick` を使用してください。
...
**生成物:** `{phase}-REVIEWS.md` — `/gtd-plan-phase --reviews` で利用可能
...
**目的:** レビュアーにはコード変更のみを表示し、GTD計画アーティファクトは含めません。

```bash
/gtd-pr-branch                     # mainに対してフィルタリング
/gtd-pr-branch develop             # developに対してフィルタリング
```
...
**999.x番号付け**により、バックログ項目はアクティブなフェーズシーケンスの外に保持されます。フェーズディレクトリは即座に作成されるため、`/gtd-discuss-phase` や `/gtd-plan-phase` がそれらに対して動作します。
...
**利用先:** `/gtd-new-milestone`（シードをスキャンしてマッチするものを提示）
...
スレッドは、複数のセッションにまたがるが特定のフェーズに属さない作業のための軽量なクロスセッション知識ストアです。`/gtd-pause-work` よりも軽量です。

```bash
/gtd-thread                         # すべてのスレッドを一覧表示
/gtd-thread fix-deploy-key-auth     # スレッドを再開
/gtd-thread "Investigate TCP timeout in pasta service"  # 新規作成
```
