...
**コマンド:** `/gtd-new-project [--auto @file.md]`
...
- 既存のコードベースコンテキスト（`/gtd-map-codebase` から取得）がある場合は読み込む

---

### 2. フェーズディスカッション

**コマンド:** `/gtd-discuss-phase [N] [--auto] [--batch]`
...
**コマンド:** `/gtd-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: フロントエンドフェーズが検出され UI-SPEC.md が存在しない場合、システムはユーザーに `/gtd-ui-phase` の実行を促さなければならない（UI セーフティゲート）
...
**コマンド:** `/gtd-audit-milestone`、`/gtd-complete-milestone`、`/gtd-new-milestone [name]`
...
**ギャップクローズ:** `/gtd-plan-milestone-gaps` は監査で特定されたギャップを埋めるためのフェーズを作成します。
...
**コマンド:** `/gtd-add-phase`、`/gtd-insert-phase [N]`、`/gtd-remove-phase [N]`
...
**コマンド:** `/gtd-quick [--full] [--discuss] [--research]`

**目的:** GTD の保証を維持しながら、より高速なパスでアドホックなタスクを実行します。
...
**目的:** 自由形式のテキストを分析し、適切な GTD コマンドにルーティングします。

**要件:**
- REQ-DO-01: システムは自然言語入力からユーザーの意図を解析しなければならない
- REQ-DO-02: システムは意図を最も適切な GTD コマンドにマッピングしなければならない
...
- REQ-NEXT-04: プロジェクトが存在しない場合、システムは `/gtd-new-project` を提案しなければならない
- REQ-NEXT-05: すべてのフェーズが完了している場合、システムは `/gtd-complete-milestone` を提案しなければならない

**状態検出ロジック:**
| 状態 | アクション |
|------|----------|
| `.planning/` ディレクトリなし | `/gtd-new-project` を提案 |
| フェーズに CONTEXT.md がない | `/gtd-discuss-phase` を実行 |
| フェーズに PLAN.md ファイルがない | `/gtd-plan-phase` を実行 |
| プランはあるが SUMMARY.md がない | `/gtd-execute-phase` を実行 |
| 実行済みだが VERIFICATION.md がない | `/gtd-verify-work` を実行 |
| すべてのフェーズが完了 | `/gtd-complete-milestone` を提案 |
...
- REQ-POSTVER-03: システムは `/gtd-verify-work` が対処すべき問題をログに記録しなければならない
...
**実行タイミング:** `/gtd-execute-phase` の検証ステップの前に自動実行されます。
...
**実行タイミング:** `/gtd-plan-phase` の末尾、プランチェッカーループの後に自動実行されます。
...
- REQ-CTX-06: コンテキストモニターは GTD アクティブ vs 非 GTD アクティブプロジェクトを区別しなければならない
...
**コマンド:** `/gtd-pause-work`、`/gtd-resume-work`、`/gtd-progress`
...
**コマンド:** `/gtd-set-profile <quality|balanced|budget|inherit>`
...
**コマンド:** `/gtd-map-codebase [area]`

**目的:** 新しいプロジェクトを開始する前に既存のコードベースを分析し、GTD が既存の構成を理解できるようにします。
...
- REQ-MAP-04: 後続の `/gtd-new-project` はコードベースマッピングを読み込み、追加する内容に焦点を当てた質問を行わなければならない
...
**コマンド:** `/gtd-add-todo [desc]`、`/gtd-check-todos`
...
**目的:** GTD を最新バージョンに更新し、チェンジログのプレビューを表示します。
...
- REQ-UPDATE-04: システムはローカルで変更されたファイルを `gtd-local-patches/` にバックアップしなければならない
- REQ-UPDATE-05: `/gtd-reapply-patches` は更新後にローカルの変更を復元しなければならない
...
**目的:** 6つの異なる AI コーディングエージェントランタイムで GTD を実行します。
...
- REQ-RUNTIME-05: アンインストールは他の設定に影響を与えることなく、すべての GTD ファイルをクリーンに削除しなければならない
...
**コマンド:** `/gtd-profile-user [--questionnaire] [--refresh]`
...
- `/gtd-dev-preferences` コマンド — 任意のセッションで好みを読み込み
...
- `gtd-user-profiler` エージェント — セッションデータからの行動分析
...
すべての `/gtd-progress` 呼び出しで、現在のマイルストーンのすべてのフェーズの未解決項目（pending、skipped、blocked、human_needed）をスキャンします。アクション可能なリンク付きのノンブロッキング警告セクションを表示します。
...
**要件:**
- REQ-DEBT-01: システムは `/gtd-progress` ですべての過去フェーズの未解決 UAT/検証項目を表面化しなければならない
...
- REQ-DEBT-06: `/gtd-audit-uat` はすべてのフェーズをスキャンし、項目をテスト可能性別にカテゴリ分けし、人的テストプランを生成しなければならない
...
**コマンド:** `/gtd-fast [task description]`
...
**`/gtd-quick` との使い分け:**
- `/gtd-fast` — 2分以内に実行可能な一文のタスク（タイポ修正、設定変更、小規模な追加）
- `/gtd-quick` — リサーチ、マルチステッププランニング、または検証が必要なもの

---

### 42. クロス AI ピアレビュー

**コマンド:** `/gtd-review --phase N [--gemini] [--claude] [--codex] [--all]`
...
- REQ-REVIEW-05: レビューは `/gtd-plan-phase --reviews` で使用可能でなければならない
...
**コマンド:** `/gtd-add-backlog <description>`、`/gtd-review-backlog`、`/gtd-plant-seed <idea>`
...
- REQ-BACKLOG-02: `/gtd-discuss-phase` と `/gtd-plan-phase` が動作するよう、フェーズディレクトリは即座に作成されなければならない
- REQ-BACKLOG-03: `/gtd-review-backlog` は項目ごとにプロモート、維持、削除のアクションをサポートしなければならない
- REQ-BACKLOG-04: プロモートされた項目はアクティブなマイルストーンシーケンスに再番号付けされなければならない
- REQ-SEED-01: シードは完全な WHY と表面化条件の WHEN をキャプチャしなければならない
- REQ-SEED-02: `/gtd-new-milestone` はシードをスキャンして一致するものを提示しなければならない
...
**コマンド:** `/gtd-thread [name | description]`

**目的:** 複数セッションにまたがるが特定のフェーズには属さない作業のための、軽量なクロスセッションナレッジストア。`/gtd-pause-work` よりも軽量 — フェーズ状態やプランコンテキストは不要です。
...
**コマンド:** `/gtd-pr-branch [target branch]`

**目的:** `.planning/` のコミットを除外して、プルリクエストに適したクリーンなブランチを作成します。レビュアーにはコード変更のみが表示され、GTD プランニング成果物は表示されません。
...
**目的:** GTD のプランニング成果物に対する多層防御セキュリティ。GTD は LLM のシステムプロンプトとなるマークダウンファイルを生成するため、これらのファイルに流入するユーザー制御テキストは間接的なプロンプトインジェクションの潜在的なベクターです。
...
**2. プロンプトインジェクションガードフック**（`gtd-prompt-guard.js`）
`.planning/` を対象とする Write/Edit 呼び出しをインジェクションパターンでスキャンする PreToolUse フック。アドバイザリーのみ — 正当な操作をブロックせず、検出を認識のためにログ記録します。

**3. ワークフローガードフック**（`gtd-workflow-guard.js`）
Claude が GTD ワークフローコンテキスト外でファイル編集を試行した際に検出する PreToolUse フック。直接編集の代わりに `/gtd-quick` や `/gtd-fast` の使用をアドバイスします。`hooks.workflow_guard`（デフォルト: false）で設定可能です。
...
**目的:** `/gtd-discuss-phase` 中に `DISCUSSION-LOG.md` を自動生成し、ディスカッション中の決定事項の完全な監査証跡を残します。
...
**コマンド:** `/gtd-forensics [description]`

**目的:** 失敗または停滞した GTD ワークフローのポストモーテム調査。
...
**コマンド:** `/gtd-milestone-summary [version]`
...
2. **切り替え** — 後続の GTD コマンドのためにアクティブなワークストリームコンテキストを変更
...
**コマンド:** `/gtd-discuss-phase`（`workflow.discuss_mode: 'assumptions'` 設定時）
...
**対象:** `/gtd-new-project` および `/gtd-progress`

**目的:** UI 重視のプロジェクトを自動検出し、`/gtd-ui-phase` の推奨を表面化します。
...
- REQ-UI-DETECT-03: システムは UI 重視フェーズのネクストステップに `/gtd-ui-phase` を提案しなければならない
- REQ-UI-DETECT-04: システムは `/gtd-ui-phase` を必須にしてはならない

**プロセス:**
1. **検出** — プロジェクト説明と技術スタックの UI シグナル（キーワード、フレームワーク参照）をスキャン
2. **アノテーション** — ROADMAP.md の該当フェーズに `ui_hint` マーカーを追加
3. **表面化** — UI 重視フェーズのネクストステップに `/gtd-ui-phase` の推奨を含める

---

### 55. マルチランタイムインストーラー選択

**対象:** `npx get-things-done-cc`
...
3. **インストール** — 1回のセッションで選択されたすべてのランタイムに対して GTD を設定
