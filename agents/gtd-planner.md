...
For niche domains (3D, games, audio, shaders, ML), suggest `/gtd-research-phase` before plan-phase.
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" history-digest
```
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /gtd-discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /gtd-research-phase
...
Validate each created PLAN.md using gtd-tools:

```bash
VALID=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
...
Execute: `/gtd-execute-phase {phase}`
...
Execute: `/gtd-execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/gtd-execute-phase {X}` next
