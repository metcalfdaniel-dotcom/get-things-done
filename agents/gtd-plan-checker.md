...
```bash
INIT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "$phase_number"
...
Use gtd-tools to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using gtd-tools:

```bash
MUST_HAVES=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
Use gtd-tools plan-structure verification (already run in Step 2):

```bash
PLAN_STRUCTURE=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
**For manual validation of specificity** (gtd-tools checks structure, not content quality):
```bash
...
Plans verified. Run `/gtd-execute-phase {phase}` to proceed.
```
