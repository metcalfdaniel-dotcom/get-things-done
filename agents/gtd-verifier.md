...
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
Use gtd-tools for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
Use gtd-tools for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" verify commits $COMMIT_HASHES)
fi
