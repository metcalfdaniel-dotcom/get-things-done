...
description: Researches how to implement a phase before planning. Produces RESEARCH.md consumed by gtd-planner. Spawned by /gsd:plan-phase orchestrator.
...
```bash
node "$HOME/.claude/get-things-done/bin/gtd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
