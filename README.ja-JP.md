...
だからGTDを作りました。複雑さはシステムの中にあり、ワークフローの中にはありません。裏側では、コンテキストエンジニアリング、XMLプロンプトフォーマッティング、サブエージェントのオーケストレーション、状態管理が動いています。あなたが目にするのは、ただ動くいくつかのコマンドだけです。

このシステムは、Claudeが仕事をし、*かつ*検証するために必要なすべてを提供します。私はこのワークフローを信頼しています。ちゃんといい仕事をしてくれます。

これがGTDです。エンタープライズごっこは一切なし。Claude Codeを使って一貫してクールなものを作るための、非常に効果的なシステムです。
...
GTDはそれを解決します。Claude Codeを信頼性の高いものにするコンテキストエンジニアリングレイヤーです。アイデアを説明し、システムに必要なすべてを抽出させ、Claude Codeに仕事をさせましょう。
...
> [!NOTE]
> Codexのインストールでは、カスタムプロンプトではなくスキル（`skills/gtd-*/SKILL.md`）を使用します。

### 最新の状態を保つ

GTDは急速に進化しています。定期的にアップデートしてください：
...
```bash
# Claude Code
npx get-things-done-cc --claude --global   # ~/.claude/ にインストール
npx get-things-done-cc --claude --local    # ./.claude/ にインストール

# OpenCode（オープンソース、無料モデル）
npx get-things-done-cc --opencode --global # ~/.config/opencode/ にインストール

# Gemini CLI
npx get-things-done-cc --gemini --global   # ~/.gemini/ にインストール

# Codex（スキルファースト）
npx get-things-done-cc --codex --global    # ~/.codex/ にインストール
npx get-things-done-cc --codex --local     # ./.codex/ にインストール

# Copilot（GitHub Copilot CLI）
npx get-things-done-cc --copilot --global  # ~/.github/ にインストール
npx get-things-done-cc --copilot --local   # ./.github/ にインストール

# Cursor CLI
npx get-things-done-cc --cursor --global      # ~/.cursor/ にインストール
npx get-things-done-cc --cursor --local       # ./.cursor/ にインストール

# Antigravity（Google、スキルファースト、Geminiベース）
npx get-things-done-cc --antigravity --global # ~/.gemini/antigravity/ にインストール
npx get-things-done-cc --antigravity --local  # ./.agent/ にインストール

# 全ランタイム
npx get-things-done-cc --all --global      # すべてのディレクトリにインストール
```
...
GTDは摩擦のない自動化のために設計されています。Claude Codeを以下のように実行してください：
...
> [!TIP]
> これがGTDの意図された使い方です — `date` や `git commit` を50回も承認するために止まっていては目的が台無しです。
...
> **既存のコードがある場合は？** まず `/gtd-map-codebase` を実行してください。並列エージェントが起動し、スタック、アーキテクチャ、規約、懸念点を分析します。その後 `/gtd-new-project` がコードベースを把握した状態で動作し、質問は追加する内容に焦点を当て、計画時にはパターンが自動的に読み込まれます。
...
> **前提モード：** 質問よりもコードベース分析を優先したい場合は、`/gtd-settings` で `workflow.discuss_mode` を `assumptions` に設定してください。システムがコードを読み、何をなぜそうするかを提示し、間違っている部分だけ修正を求めます。詳しくは[ディスカスモード](docs/ja-JP/workflow-discuss-mode.md)をご覧ください。
...
すべてパスすれば次に進みます。何か壊れていれば、手動でデバッグする必要はありません — 作成された修正プランで `/gtd-execute-phase` を再度実行するだけです。
...
/gtd-execute-phase 2
/gtd-verify-work 2
/gtd-ship 2                  # 検証済みの作業からPRを作成
...
/gtd-complete-milestone
/gtd-new-milestone
```

またはGTDに次のステップを自動判定させます：

```
/gtd-next                    # 次のステップを自動検出して実行
```

**discuss → plan → execute → verify → ship** のループをマイルストーン完了まで繰り返します。

ディスカッション中のインプットを速くしたい場合は、`/gtd-discuss-phase <n> --batch` で1つずつではなく小さなグループにまとめた質問に一括で回答できます。

各フェーズであなたのインプット（discuss）、適切なリサーチ（plan）、クリーンな実行（execute）、人間による検証（verify）が行われます。コンテキストは常にフレッシュ。品質は常に高い。

すべてのフェーズが完了したら、`/gtd-complete-milestone` でマイルストーンをアーカイブしリリースをタグ付けします。

次に `/gtd-new-milestone` で次のバージョンを開始します — `new-project` と同じフローですが既存のコードベース向けです。次に構築したいものを説明し、システムがドメインを調査し、要件をスコーピングし、新しいロードマップを作成します。各マイルストーンはクリーンなサイクルです：定義 → 構築 → シップ。
...
クイックモードはGTDの保証（アトミックコミット、状態トラッキング）をより速いパスで提供します：
...
| コマンド | 説明 |
|---------|--------------|
| `/gtd-new-project [--auto]` | フル初期化：質問 → リサーチ → 要件定義 → ロードマップ |
| `/gtd-discuss-phase [N] [--auto] [--analyze]` | 計画前に実装の決定事項をキャプチャ（`--analyze` でトレードオフ分析を追加） |
| `/gtd-plan-phase [N] [--auto] [--reviews]` | フェーズのリサーチ + プラン + 検証（`--reviews` でコードベースレビューの発見事項を読み込み） |
| `/gtd-execute-phase <N>` | 全プランを並列ウェーブで実行し、完了時に検証 |
| `/gtd-verify-work [N]` | 手動ユーザー受入テスト ¹ |
| `/gtd-ship [N] [--draft]` | 検証済みのフェーズ作業から自動生成された本文付きのPRを作成 |
| `/gtd-next` | 次の論理的なワークフローステップに自動的に進む |
| `/gtd-fast <text>` | インラインの軽微タスク — 計画を完全にスキップし即座に実行 |
| `/gtd-audit-milestone` | マイルストーンが完了の定義を達成したか検証 |
| `/gtd-complete-milestone` | マイルストーンをアーカイブし、リリースをタグ付け |
| `/gtd-new-milestone [name]` | 次のバージョンを開始：質問 → リサーチ → 要件定義 → ロードマップ |
| `/gtd-forensics [desc]` | 失敗したワークフロー実行の事後分析（停止ループ、欠落成果物、git異常の診断） |
| `/gtd-milestone-summary [version]` | チームオンボーディングとレビュー向けの包括的なプロジェクトサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/gtd-workstreams list` | 全ワークストリームとそのステータスを表示 |
| `/gtd-workstreams create <name>` | 並列マイルストーン作業用の名前空間付きワークストリームを作成 |
| `/gtd-workstreams switch <name>` | アクティブなワークストリームを切り替え |
| `/gtd-workstreams complete <name>` | ワークストリームを完了しマージ |

### マルチプロジェクトワークスペース

| コマンド | 説明 |
|---------|--------------|
| `/gtd-new-workspace` | リポジトリのコピー（worktreeまたはクローン）で隔離されたワークスペースを作成 |
| `/gtd-list-workspaces` | すべてのGTDワークスペースとそのステータスを表示 |
| `/gtd-remove-workspace` | ワークスペースを削除しworktreeをクリーンアップ |

### UIデザイン

| コマンド | 説明 |
|---------|--------------|
| `/gtd-ui-phase [N]` | フロントエンドフェーズ用のUIデザイン契約（UI-SPEC.md）を生成 |
| `/gtd-ui-review [N]` | 実装済みフロントエンドコードの6つの柱によるビジュアル監査（遡及的） |

### ナビゲーション

| コマンド | 説明 |
|---------|--------------|
| `/gtd-progress` | 今どこにいる？次は何？ |
| `/gtd-next` | 状態を自動検出し次のステップを実行 |
| `/gtd-help` | 全コマンドと使い方ガイドを表示 |
| `/gtd-update` | チェンジログプレビュー付きでGTDをアップデート |
| `/gtd-join-discord` | GTD Discordコミュニティに参加 |
| `/gtd-manager` | 複数フェーズ管理用のインタラクティブコマンドセンター |

### ブラウンフィールド

| コマンド | 説明 |
|---------|--------------|
| `/gtd-map-codebase [area]` | new-project前に既存のコードベースを分析 |

### フェーズ管理

| コマンド | 説明 |
|---------|--------------|
| `/gtd-add-phase` | ロードマップにフェーズを追加 |
| `/gtd-insert-phase [N]` | フェーズ間に緊急作業を挿入 |
| `/gtd-remove-phase [N]` | 将来のフェーズを削除し番号を振り直し |
| `/gtd-list-phase-assumptions [N]` | 計画前にClaudeの意図するアプローチを確認 |
| `/gtd-plan-milestone-gaps` | 監査で見つかったギャップを埋めるフェーズを作成 |

### セッション

| コマンド | 説明 |
|---------|--------------|
| `/gtd-pause-work` | フェーズ途中で停止する際の引き継ぎを作成（HANDOFF.jsonを書き込み） |
| `/gtd-resume-work` | 前回のセッションから復元 |
| `/gtd-session-report` | 実行した作業と結果のセッションサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/gtd-workstreams` | 並列ワークストリームを管理（list、create、switch、status、progress、complete） |

### コード品質

| コマンド | 説明 |
|---------|--------------|
| `/gtd-review` | 現在のフェーズまたはブランチのクロスAIピアレビュー |
| `/gtd-pr-branch` | `.planning/` コミットをフィルタリングしたクリーンなPRブランチを作成 |
| `/gtd-audit-uat` | 検証負債を監査 — UATが未実施のフェーズを検出 |

### バックログ & スレッド

| コマンド | 説明 |
|---------|--------------|
| `/gtd-plant-seed <idea>` | トリガー条件付きの将来志向のアイデアをキャプチャ — 適切なマイルストーンで浮上 |
| `/gtd-add-backlog <desc>` | バックログのパーキングロットにアイデアを追加（999.xナンバリング、アクティブシーケンス外） |
| `/gtd-review-backlog` | バックログ項目をレビューし、アクティブマイルストーンに昇格またはstaleエントリを削除 |
| `/gtd-thread [name]` | 永続コンテキストスレッド — 複数セッションにまたがる作業用の軽量クロスセッション知識 |

### ユーティリティ

| コマンド | 説明 |
|---------|--------------|
| `/gtd-settings` | モデルプロファイルとワークフローエージェントを設定 |
| `/gtd-set-profile <profile>` | モデルプロファイルを切り替え（quality/balanced/budget/inherit） |
| `/gtd-add-todo [desc]` | 後で取り組むアイデアをキャプチャ |
| `/gtd-check-todos` | 保留中のtodoを一覧表示 |
| `/gtd-debug [desc]` | 永続状態を持つ体系的デバッグ |
| `/gtd-do <text>` | フリーフォームテキストを適切なGTDコマンドに自動ルーティング |
| `/gtd-note <text>` | ゼロフリクションのアイデアキャプチャ — ノートの追加、一覧、todoへの昇格 |
| `/gtd-quick [--full] [--discuss] [--research]` | GTDの保証付きでアドホックタスクを実行（`--full` でプランチェックと検証を追加、`--discuss` で事前にコンテキストを収集、`--research` で計画前にアプローチを調査） |
| `/gtd-health [--repair]` | `.planning/` ディレクトリの整合性を検証、`--repair` で自動修復 |
| `/gtd-stats` | プロジェクト統計を表示 — フェーズ、プラン、要件、gitメトリクス |
| `/gtd-profile-user [--questionnaire] [--refresh]` | セッション分析から開発者行動プロファイルを生成し、パーソナライズされた応答を提供 |
...
GTDはプロジェクト設定を `.planning/config.json` に保存します。`/gtd-new-project` 実行時に設定するか、後から `/gtd-settings` で更新できます。完全な設定スキーマ、ワークフロートグル、gitブランチオプション、エージェントごとのモデル内訳については、[ユーザーガイド](docs/ja-JP/USER-GUIDE.md#configuration-reference)をご覧ください。
...
これらのトグルには `/gtd-settings` を使用するか、呼び出し時にオーバーライドできます：
...
**戦略：**
- **`none`** — 現在のブランチにコミット（デフォルトのGTD動作）
- **`phase`** — フェーズごとにブランチを作成し、フェーズ完了時にマージ
- **`milestone`** — マイルストーン全体で1つのブランチを作成し、完了時にマージ

マイルストーン完了時、GTDはスカッシュマージ（推奨）または履歴付きマージを提案します。
...
- **PreToolUseプロンプトガードフック** — `gtd-prompt-guard` が `.planning/` への書き込みに埋め込まれたインジェクションベクトルをスキャン（アドバイザリー、ブロッキングではない）
...
> [!NOTE]
> GTDはLLMシステムプロンプトとなるマークダウンファイルを生成するため、計画成果物に流入するユーザー制御テキストは潜在的な間接プロンプトインジェクションベクトルとなります。これらの保護は、そのようなベクトルを複数のレイヤーで捕捉するように設計されています。

### 機密ファイルの保護

GTDのコードベースマッピングおよび分析コマンドは、プロジェクトを理解するためにファイルを読み取ります。**シークレットを含むファイルを保護する**には、Claude Codeの拒否リストに追加してください：
...
> [!IMPORTANT]
> GTDにはシークレットのコミットに対する組み込み保護がありますが、多層防御がベストプラクティスです。防御の第一線として、機密ファイルへの読み取りアクセスを拒否してください。
...
- Codexの場合、`~/.codex/skills/gtd-*/SKILL.md`（グローバル）または `./.codex/skills/gtd-*/SKILL.md`（ローカル）にスキルが存在するか確認してください

**コマンドが期待通りに動作しない？**
- `/gtd-help` を実行してインストールを確認してください
- `npx get-things-done-cc` を再実行して再インストールしてください
...
これにより、他の設定を保持しながら、すべてのGTDコマンド、エージェント、フック、設定が削除されます。

---

## コミュニティポート

OpenCode、Gemini CLI、Codexは `npx get-things-done-cc` でネイティブサポートされています。

以下のコミュニティポートがマルチランタイムサポートの先駆けとなりました：

| プロジェクト | プラットフォーム | 説明 |
|---------|----------|-------------|
| [gtd-opencode](https://github.com/rokicool/gtd-opencode) | OpenCode | オリジナルのOpenCode対応版 |
| gtd-gemini（アーカイブ済み） | Gemini CLI | uberfuzzyによるオリジナルのGemini対応版 |
...
**Claude Codeは強力です。GTDはそれを信頼性の高いものにします。**
